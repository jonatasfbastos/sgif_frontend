...
function filtrar() {
  ...
  var opcao =
    selectedIdDisciplina.options[selectedIdDisciplina.selectedIndex].value;
  if (selectedDtInicio == "") {
...
function tableCreate(data) {
  ...
  if (tableBody) {
    ...
    data.forEach((element) => {
      ...
      colDisciplina.appendChild(
        document.createTextNode(
          element.disciplina ? element.disciplina.nome : ""
        )
      );
...
function openEditPopup(id) {
  ...
  document.getElementById("avaliacaoDataInicioEditar").value = avl.dataInicio
    .split("/")
    .reverse()
    .join("-");
  document.getElementById("avaliacaoDataFimEditar").value = avl.dataFim
    .split("/")
    .reverse()
    .join("-");
...
function adicionar() {
  this.avaliacao.descricao = document.getElementById(
    "avaliacaoDescricaoAdd"
  ).value;
  this.avaliacao.dataInicio = document.getElementById(
    "avaliacaoDataInicioAdd"
  ).value;
  this.avaliacao.dataFim = document.getElementById("avaliacaoDataFimAdd").value;
  this.avaliacao.disciplina = {
    id: document.getElementById("disciplinaAdd").value,
  };

  this.avaliacao.dataInicio = this.avaliacao.dataInicio
    .split("-")
    .reverse()
    .join("/");
  this.avaliacao.dataFim = this.avaliacao.dataFim
    .split("-")
    .reverse()
    .join("/");
...
function editar() {
  ...
  if (
    ...
    popupErroExibir(
      "Os dados estão repetidos. Por favor, altere os dados necessários"
    );
    this.avaliacao = {};
    return;
  }
